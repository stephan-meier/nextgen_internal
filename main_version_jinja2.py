...
vRet, vSubdirs = getSubdirs(source_dir)
if vRet==0:
    print(vSubdirs)
    if len(vSubdirs)>0:
        for sd in vSubdirs:
            if sd == run_templates:
                vRet2, vFiles = getFiles(source_dir + sd + '/')
                if vRet2==0:
                    print(vFiles)
                    if tasks_name in vFiles:
                        #YAML File auseinandernehmen
                        with open(source_dir + run_templates + '/' + tasks_name, 'r') as f:
...
table_select = '*'     #'dim_Study.json'
for run in list_tables:
    if (table_select == '*') or (table_select == run):
        src_table = {}
        src_table = read_template(source_dir + run)
        if src_table != {}:
            print("--------------------------------------------------")
            print(" RUN: ", run)
            print("--------------------------------------------------")
