...
def execute_statements(sql_script):
    ...
    if doExecute:
        ...
        try:
            ...
            print('pyodbc: Data Error!')
            ret = 1
        except pyodbc.OperationalError as err:
            print('pyodbc: Operational Error!')
            ret = 1
        except pyodbc.IntegrityError as err:
            print('pyodbc: Integrity Error!')
            ret = 1
        except pyodbc.InternalError as err:
            print('pyodbc: Internal Error!')
            ret = 1
        except pyodbc.ProgrammingError as err:
            print('pyodbc: Programming Error!')
            ret = 1
        except pyodbc.NotSupportedError as err:
            print('pyodbc: NotSupported Error!')
            ret = 1
        else:
            cursor.close()
            conn.close()
...
def build_query(template, schema_name, p_src_table, p_ts):
    ...
    try:
        ...
        logger.error("In build_query gibts einen Fehler. Template: ", template)
    return(script)
