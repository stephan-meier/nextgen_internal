...
def sta_90er_view_PLAN_B(schema_name, table_name, p_bk_column, p_column_list):    
...
begin
    declare @view_exists int
    declare @sql nvarchar(max)
    select @view_exists = 1 from sysobjects where id = object_id('{{v_view_name}}') and type = 'V'
    if @view_exists is null
    begin
        SET @sql = 'create view {{v_view_name}} as
        select
            {{v_bk_column["column_name"]}} = CAST(NULL {{v_bk_column["data_type"]}})
            {%- for feld in v_column_list %}
            ,{{feld["column_name"]}} = CAST(NULL as {{feld["data_type"]}})
            {%- endfor %}
        where 1=1
        '
        exec sp_executesql @sql
...
"""
    ...
    script = template.render(v_view_name=view_name, v_bk_column=p_bk_column[0], v_column_list=p_column_list)
...
sta_90er_view_PLAN_B(sta_schema_name, table_name, src_bk_column, src_column_list)
